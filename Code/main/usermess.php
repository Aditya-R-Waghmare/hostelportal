		...
		<div class="row-fluid">
			...
			<div class="span10">
				<div class="contentheader">
					<i class="icon-table"></i> Mess Information
				</div>
				<ul class="breadcrumb">
				...
				<div style="margin-top: -19px; margin-bottom: 21px;">
					<a href="user.php"><button class="btn btn-default btn-large" style="float: left;"><i
				...
				$result = $db->prepare("SELECT usn FROM students WHERE sname='$name';");
				$result->execute();
				$row = $result->fetch();
				?>
				<form action="saveuserbreakfast.php" method="post" enctype="multipart/form-data">

					<hr>
					<center>
						<div id="ac">
							<h3>Select Breakfast:</h3>
							<input type="hidden" name="usn" value="<?php echo $row[0]; ?>" />
							<input type="hidden" name="sname" value="<?php echo $name ?>" />

							<select name="bfast"
								style=" background-color: whitesmoke; width:265px; height:30px; margin-left:-5px;"
								required>
							...
							<div>
								...
								<button class="btn btn-success btn-block btn-large" style="width:267px;"><i
										class="icon icon-save icon-large"></i>Submit</button>
							</div>
						</div>
				</form>
